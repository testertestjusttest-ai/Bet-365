...
export async function PATCH(req:Request){
 const a=await actor();if(!a)return NextResponse.json({error:"Unauthorized"},{status:401});if(a.admin_role!=="main_admin")return NextResponse.json({error:"Main Admin only"},{status:403});
 const body=await req.json();if(!body.id||!["user","support_admin","admin","main_admin"].includes(body.admin_role))return NextResponse.json({error:"Invalid role"},{status:400});
