...
export async function GET(req:NextRequest,{params}:{params:Promise<{sport:string;id:string}>}){
 try{
  ...
  const scoreRows=Array.isArray(score?.scores)?score.scores:[];
  const homeScoreEntry=scoreRows.find((x:any)=>x.name===event.home_team);
  const awayScoreEntry=scoreRows.find((x:any)=>x.name===event.away_team);
  const hasLiveScore=Boolean(homeScoreEntry&&awayScoreEntry);
  const completed=Boolean(score?.completed);
  const status=completed?"finished":hasLiveScore?"live":"scheduled";
  ...
  const markets=(bookmaker.markets||[]).map((m:any)=>({
   ...
   market_type:m.key,
   active:true,
   selections:(m.outcomes||[]).filter((o:any)=>typeof o.price==="number"&&o.price>1).map((o:any)=>({
    id:outcomeId(m.key,o.name,o.point),
    label:o.point==null?o.name:o.name+" "+(o.point>0?"+":"")+o.point,
    odds:Number(o.price),
    status:"open",
    point:o.point
   }))
