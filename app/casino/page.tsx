...
export default function Casino(){
 ...
 useEffect(()=>{
   ...
   fetch("/api/casino/catalog").then(async r=>{
     ...
     setCatalogMessage(error instanceof Error?error.message:"Casino catalog unavailable");
   });
 ...
 const allProviders=useMemo(()=>{
   ...
   const known=new Set(merged.map(p=>p.name.toLowerCase()));
   for(const remote of remoteProviders){if(!known.has(remote.name.toLowerCase()))merged.push(remote);}
   return merged;
 },[remoteProviders]);
  ...
  <div className="sports-page casino-page">
   ...
   <div className="casino-provider-grid">
    {filtered.map((provider,index)=><a className="casino-provider-card" href={"/casino?provider="+encodeURIComponent(provider.name)} key={provider.name}>
      ...
      <div><b>{provider.full}</b><small>{provider.type}{provider.gameCount?" • "+provider.gameCount+" games":""}</small></div><ChevronRight size={19}/>
    </a>)}
   </div>

   {catalogState==="configured"&&filteredGames.length>0&&<section className="casino-games-section">
    <div className="section-head"><div><span className="eyebrow">CONNECTED CATALOG</span><h2>{selected?selected+" games":"Available games"}</h2></div></div>
    <div className="casino-game-grid">
      {filteredGames.slice(0,60).map(game=><article className="casino-game-card" key={game.id}>
        {game.thumbnail_url?<img src={game.thumbnail_url} alt="" loading="lazy"/>:<div className="casino-game-placeholder">{initials(game.name)}</div>}
        <div className="casino-game-info"><b>{game.name}</b><small>{game.provider_code+" • "+(game.game_type||"Casino")+(game.rtp?" • RTP "+game.rtp+"%":"")}</small></div>
        <span className="casino-demo-badge">{game.has_demo?"Demo":"Catalog"}</span>
      </article>)}
    </div>
