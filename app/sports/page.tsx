...
export default function Sports(){
 ...
 async function load(next=0){setLoading(true);try{const r=await fetch("/api/events?sport="+encodeURIComponent(active)+"&page="+next+"&pageSize=48");const j=await r.json();if(!r.ok)throw new Error(j.error||"Events unavailable");setSource(j.source||"database");if(next===0)setEvents(j.events||[]);else setEvents(x=>[...x,...(j.events||[])]);setHasMore(Boolean(j.hasMore));if(j.sports?.length)setSports(x=>[...new Set([...x,...j.sports])]);setPage(next);}finally{setLoading(false)}}
