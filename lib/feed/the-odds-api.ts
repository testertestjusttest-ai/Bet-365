...
export async function fetchOdds(sport:string,marketsOverride?:string){
  const c=feedConfig();
  const q=new URLSearchParams({
    apiKey:c.apiKey,
    regions:c.regions,
    markets:marketsOverride||c.markets,
    oddsFormat:"decimal",
    dateFormat:"iso"
  });
...
export async function fetchScores(sport:string){
  ...
  return getJson<Array<{
    id:string;sport_key:string;sport_title?:string;commence_time:string;completed:boolean;
    home_team:string;away_team:string;scores?:Array<{name:string;score:string}>;last_update?:string
  }>>(BASE+"/sports/"+encodeURIComponent(sport)+"/scores?"+q.toString());
}
